...
{{-- wire:key as it is dynamic and without it it makes potential issues
...
<div wire:key="{{$todo->id}}" class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
            <div class="flex justify-between space-x-2">
                  <div class="flex items-center">
                    @if ($todo->completed)
                    <input wire:click="toggle({{$todo->id}})" class="mr-2" type="checkbox" name="completed" checked>

                    @else

                    <input wire:click="toggle({{$todo->id}})" class="mr-2" type="checkbox" name="completed" >
                    @endif
<div>
